...
subsetting <- function(seurat_object, csv){
    csv <- read.csv(csv)
    barcodes <- csv[1]
    seurat_object@meta.data$rownames <- rownames(seurat_object@meta.data) 
    subset(seurat_object, subset = rownames %in% rownames) -> seurat_object
    seurat_object@meta.data$rownames <- NULL
    return (seurat_object)
}
