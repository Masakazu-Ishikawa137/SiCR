...
dimensional_plotServer <- function(id, myReactives) {
  moduleServer(id, function(input, output, session) {
    ...
    # render_plot_and_download_pdf(output, 'plot', reactive({myReactives$dimplot}), plot_width, plot_height)

    render_plot(
      output,
      "plot",
      reactive({myReactives$dimplot}),
      reactive({input$plot_width}),
      reactive({input$plot_height})
    )
